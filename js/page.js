...
var xmlHttp
function GetXmlHttpObject_page(handler)
{ 
	var objXmlHttp=null
...
function pool1_ajx_page(siteurl)
{
	...
	document.getElementById("pool1_msg_page").innerHTML="loading...";
	var date_now=new Date()
    var mynumber=Math.random()
	var url=siteurl+"/pool-one-wp-answer.php?ans="+ ans.value + "&timestamp=" + date_now + "&action=" + mynumber;
    xmlHttp=GetXmlHttpObject_page(newchanged_ncc_page)
    xmlHttp.open("GET", url , true)
    xmlHttp.send(null)
	
}

function newchanged_ncc_page() 
{ 
	...
	{ 
		...
		{
			document.getElementById("pool1_page").innerHTML = (xmlHttp.responseText).trim();
		}
	} 
} 
